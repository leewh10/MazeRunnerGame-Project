...
public class GameScreen implements Screen {
    ...
    private final float screenShakeDuration = 0.5f;
    private final float screenShakeIntensity = 2f;
    private final Vector2 originalCameraPosition = new Vector2();  // Store the original camera position for restoration
     ...
     * Otherwise, everytime a coordinate needed to be called, a class would have to be called first
     */
    ...
    public void render(float delta) {
        if (!isPaused) {
            ...
            /**
             * If the character has 0 or fewer lives, the background music is stopped and the screen is changed to the GameOverScreen.
             */
                ...
                for (int y = 0; y <= maxY; y++) {
                    ...
                    switch (variable) {
                        ...
                        case 3:
                             ...
                             * Immunity to fire
                             */
                            if (character.collidesWithTrap(mazeX, mazeY) && !Character.isIsImmuneToFire()) {
                                /**
                        ...
                        case 10:
                            ...
                            /**
                             * If the character collides with the lever, the wall is removed
                             */
                ...
                if ((Character.isCollidedTree() && isEnemyVisible) || !Character.isCollidedTree()) {
                    ...
                    if (character.collidesWithEnemy(enemy.getX(), enemy.getY())) {
                        if (Gdx.input.isKeyPressed(Input.Keys.SPACE)) { //The condition for killing enemies
                            ...
                            removeFromMazeData(enemy.getX(), enemy.getY(),4,14);
            ...
            /**
             * render the Devil and when the character and the Devil collide, the corresponding functionalities are called
             */
     ...
     * This way, even after the object is removed, we can work with it. An example is the movable wall crumbling after it has been removed.
     ...
     * Initially, the original camera position is saved.
     * @return
     */
    ...
    /**
     * The screen shakes by shifting the camera position between the described shakeAmount
     * If the total shake time exceeds the defined duration, the screen shaking is stopped.
